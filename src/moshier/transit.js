...
/* Calculate time of transit assuming RA and Dec change uniformly with time */
export const calc = function(date, lha, dec, result) {
  ...
  const xx1 = input.glat * constant.DTR;
  const coslat = Math.cos(xx1);
  const sinlat = Math.sin(xx1);
  ...
  const xx = Math.floor(date.universal - 0.5) + 0.5; // UT
  const x = (date.universal - xx) * constant.TPI; // UT
  ...
  if (!(coslat == 0.0 || cosdec == 0.0)) {
    ...
    switch (variable.body.key) {
      ...
      case 'moon': {
        ...
        const D = Math.asin(N); /* the parallax */
    ...
    if (y < 1.0 && y > -1.0) {
       ...
       */
      let z = -variable.ddecdt * (sinlat + transit.COSZEN * sindec);
      z /= constant.TPI * coslat * cosdec * cosdec;
      /* Derivative of acos(y): */
      z /= Math.sqrt(1.0 - y * y);
      y = Math.acos(y);
      const D = -variable.dradt / constant.TPI + 1.00273790934;
...
export const searchHalve = function(t1, y1, t2, y2, callback) {
  let e2 = y2 - transit.elevation_threshold;
  ...
  while (Math.abs(t2 - t1) > 0.00001) {
    ...
    const ym = altaz.elevation;
    const em = ym - transit.elevation_threshold;
...
export const noRiseSet = function(t0, callback) {
  const t_trnsit0 = transit.t_trnsit; // double
  const el_trnsit0 = transit.elevation_trnsit; // double
  ...
  let t_above = t_trnsit0;
  let el_above = el_trnsit0;
  let t_below = -1.0;
  let el_below = el_above;
  let t = t_trnsit0 - 0.25;
  let e = 1.0;
  while (e > 0.005) {
    iterator(t, callback);
...
export const iterateTransit = function(callback, result) {
  ...
  let t1 = bodies.earth.position.date.universal; // UT
  ...
  if (!(loopctr > 10)) {
    ...
    const trnsit1 = transit.r_trnsit;
    const set1 = transit.r_set;
    if (transit.f_trnsit == 0) {
      ...
      let isPrtrnsit = false;
      const date_trnsit = date;
