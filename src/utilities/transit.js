import { DTR, RTD, TPI, DISFAC, COSSUN, COSZEN } from '../constants'
...
transit.calc = function (date, lha, dec, constant, body, observer, result) {
	...
	lhay = y;
	y =  y/( -constant.dradt / TPI + 1.00273790934);
	this.r_trnsit = x - y;
	...
	if( !((coslat == 0.0) || (cosdec == 0.0)) ) {
		...
		if( (y < 1.0) && (y > -1.0) ) {
			 ...
			 */
			z = -constant.ddecdt * (sinlat + COSZEN * sindec);
			z /= TPI*coslat*cosdec*cosdec;
			/* Derivative of acos(y): */
			z /= Math.sqrt( 1.0 - y*y);
			y = Math.acos(y);
			D = -constant.dradt / TPI + 1.00273790934;
